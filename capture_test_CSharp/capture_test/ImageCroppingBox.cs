    ...
    public class ImageCroppingBox : Control
    {
            ...
            if (this._IsSelected) {//如果已经绘制 移动过程中判断是否需要设置鼠标样式
                ...
                if (m_bMoving) {//如果是移动选区 判断选区移动范围
                    ...
                    if (this._IsSetClip) {
                        if (tempX < 0) tempX = 0;
                        if (tempY < 0) tempY = 0;
                        if (this._SelectedRectangle.Width + tempX >= m_rectClip.Width)
                            tempX = m_rectClip.Width - this._SelectedRectangle.Width - 1;
                        if (this._SelectedRectangle.Height + tempY >= m_rectClip.Height)
                            tempY = m_rectClip.Height - this._SelectedRectangle.Height - 1;
                    }
        ...
        protected virtual void DrawMagnifier(Graphics g, Point pt) {
            ...
            if (rect.Right > this.Width)
                rect.X = pt.X - rect.Width - 10;
            if (rect.Bottom > this.Height)
                rect.Y = pt.Y - rect.Height - 10;
            //区域背景和图像先刷上去
            ...
            //反正下面就是各种巴拉巴拉的装饰、、、
            using (Pen pen = new Pen(Color.FromArgb(125, 0, 255, 255), 3)) {
